...
class PDFDocumentWithTables extends PDFDocument {
	...
	table(table, options, callback) {
		return new Promise((resolve, reject) => {
			try {
				...
				const prepareHeader =
					...
					(() =>
						this.fillColor("black").font("Sarabun-Bold").fontSize(8).fill());
				const prepareRow =
					options.prepareRow ||
					((row, indexColumn, indexRow, rectRow, rectCell) =>
						this.fillColor("black").font("Sarabun").fontSize(8).fill());
				//const prepareCell      = options.prepareCell || ((cell, indexColumn, indexRow, indexCell, rectCell) => this.fillColor('black').font("Sarabun").fontSize(8).fill());
        ...
        // regis new font
        this.registerFont("Sarabun", "./Sarabun/Sarabun-Regular.ttf");
        this.registerFont("Sarabun-Bold", "./Sarabun/Sarabun-Bold.ttf");
				...
				const createTitle = (data, size, opacity) => {
					...
					if (typeof data === "string") {
						...
						// title object
						data.label &&
							this.fontSize(data.fontSize || size).text(
								data.label,
								startX,
								startY
							);
				...
				const prepareRowBackground = (row, rect) => {
					...
					// add backgroundColor
					if (row.hasOwnProperty("columnColor")) {
						// ^0.1.70

						const { columnColor, columnOpacity } = row;
						fill = columnColor;
						opac = columnOpacity;
					} else if (row.hasOwnProperty("backgroundColor")) {
						// ~0.1.65 old

						const { backgroundColor, backgroundOpacity } = row;
						fill = backgroundColor;
						opac = backgroundOpacity;
					} else if (row.hasOwnProperty("background")) {
						// dont remove
				...
				const addHeader = () => {
					...
					const calc =
						startY +
						this.titleHeight +
						1.2 * (this.headerHeight + this.firstLineHeight);
				...
				table.datas.forEach((row, i) => {
					...
					// else this.addPage();
					if (startY + 2 * rowHeight >= maxY && !this.lockAddPage)
						this.addPage();
					...
					table.headers.forEach((dataHeader, index) => {
						let { property, width, renderer, align, valign, padding } =
							dataHeader;
						...
						if (String(text).indexOf("size") === 0) {
							let size =
								String(text).substr(4, 2).replace(":", "").replace("+", "") >>
								0;
				...
				table.rows.forEach((row, i) => {
					...
					row.forEach((cell, index) => {
						...
						if (typeof table.headers[index] === "object") {
							...
							// align
							table.headers[index].align &&
								(align = table.headers[index].align);
							table.headers[index].valign &&
								(valign = table.headers[index].valign);
						}
		...
		return new Promise((resolve, reject) => {
			try {
				...
				Array.isArray(tables)
					? // for each on Array
					  tables.forEach(
							async (table) => await this.table(table, table.options || {})
					  )
